    ...
    return (
      <View style={{ padding: 10, backgroundColor: '#FFF', borderRadius: 10 }} >
        <View>
          ...
          <Text>maintenance_date :{item?.maintenance_date}</Text>
          <Text>Appunti: {item?.notes}</Text>
        </View>
      </View>
    );
  };
...
const AddMaintinace=()=>{
  ...
  return (
    <View>
      ...
      {activeSheet==="List"?
      <>
      <Text style={{ alignSelf: 'center', fontSize: 20, fontWeight: '500',color:'black',top:10 }}>
         AssetsMaintance list </Text>
      {item?.length === 0 ? <NoDataFound title="No Data Found"/> :
        ...
        <View >
          <ScrollView>
            <View>
              <Text style={{alignSelf:'center',fontSize:20,fontWeight:'800',color:'black'}}>Manutenzione Form </Text>
            </View>
